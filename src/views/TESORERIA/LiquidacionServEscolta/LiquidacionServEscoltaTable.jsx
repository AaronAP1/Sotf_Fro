...
function LiquidacionServEscoltaTable() {
    const [liquidacionServEscoltaData, setLiquidacionServEscoltaData] = useState([]);
    const [isOpen, setIsOpen] = useState(false);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("http://localhost:3000/LiquidacionServEscolta");
                setLiquidacionServEscoltaData(response.data);
            } catch (error) {
                console.error("Error al obtener los datos:", error);
                setLiquidacionServEscoltaData([]);
            }
        };
        fetchData();
    }, []);
                ...
                <div className="flex justify-between items-center mb-4 gap-3">
                    <div className="flex items-center gap-2">
                        <div className="text-black text-lg sm:text-2xl font-normal font-poppins">
                            Liquidación de Servicios de Escolta
                        </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-5 gap-4">
                        ...
                        <div className="flex flex-col">
                            ...
                            <select className="p-2 border border-gray-300 rounded-lg">
                                <option>Vijaes sin Liquidar</option>
                                <option>1</option>
                                <option>2</option>
                            </select>
                        </div>
                    ...
                    <table className="min-w-full bg-white rounded-lg border-t border-gray-300">
                        <thead className="bg-[#f4f5fb] border-b border-gray-300">
                            <tr className="text-xs sm:text-sm font-semibold text-[#00535e]">
                                ...
                                <th className="px-4 py-2">Importe Pagado</th>
                                <th className="px-4 py-2">Fecha Detracción</th>
                                <th className="px-4 py-2">Total Detracción</th>
                                <th className="px-4 py-2">% Detracción</th>
                                <th className="px-4 py-2">Banco Detracción</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-gray-200">
                            {liquidacionServEscoltaData.length > 0 ? liquidacionServEscoltaData.map((liquidacionServEscolta) => (
                                <tr key={liquidacionServEscolta.id} className="text-xs sm:text-sm text-gray-700">
                                    ...
                                    <td className="px-4 py-2">{liquidacionServEscolta.fecha}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.estado}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.proveedor}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.viaje}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.moneda}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.totalServicio}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.importePagado}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.fechaDetraccion}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.totalDetraccion}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.detraccion}</td>
                                    <td className="px-4 py-2">{liquidacionServEscolta.bancoDetraccion}</td>
                                </tr>
                            )) : (
                                <tr>
...
export default LiquidacionServEscoltaTable;