...
import NewPagoOrdenes from "./NewPagoOrdenes";

function PagoOrdenesTable() {
    const [pagoOrdenesData, setPagoOrdenesData] = useState([]);
    const [isOpen, setIsOpen] = useState(false);

    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await axios.get("http://localhost:3000/PagoOrdenes");
                setPagoOrdenesData(response.data);
            } catch (error) {
    ...
    return (
        <div className="flex-1 bg-[#F4F5FB] h-full p-4">
                    ...
                    <table className="min-w-full bg-white rounded-lg border-t border-gray-300">
                        <thead className="bg-[#f4f5fb] border-b border-gray-300">
                            <tr className="text-xs sm:text-sm font-semibold text-[#00535e]">
                                ...
                                <th className="px-4 py-2">Órden</th>
                                <th className="px-4 py-2">Total Órden</th>
                                <th className="px-4 py-2">Monto Pago</th>
                        ...
                        <tbody className="divide-y divide-gray-200">
                            {pagoOrdenesData.length > 0 ? pagoOrdenesData.map((pagoOrdenes) => (
                                <tr key={pagoOrdenes.id} className="text-xs sm:text-sm text-gray-700">
                                    ...
                                    <td className="px-4 py-2">{pagoOrdenes.fecha}</td>
                                    <td className="px-4 py-2">{pagoOrdenes.orden}</td>
                                    <td className="px-4 py-2">{pagoOrdenes.totalOrden}</td>
                                    <td className="px-4 py-2">{pagoOrdenes.montoPago}</td>
                                    <td className="px-4 py-2">{pagoOrdenes.tipoCanal}</td>
                                    <td className="px-4 py-2">{pagoOrdenes.medioPago}</td>
                                    <td className="px-4 py-2">{pagoOrdenes.numeroOperacion}</td>
                                    <td className="px-4 py-2">{pagoOrdenes.observacion}</td>
                                </tr>
                            )) : (
                                <tr>
            ...
            <NewPagoOrdenes isOpen={isOpen} onClose={closeModal} />
        </div>
    );
}

export default PagoOrdenesTable;