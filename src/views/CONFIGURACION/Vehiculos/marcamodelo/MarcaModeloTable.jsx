...
import NewMarcaModelo from './NewMarcaModelo';
import { RiFileExcel2Line } from 'react-icons/ri';
import useMarcaModelos from '../../../../Hooks/UseMarcaModeloVeh';


function MarcaModeloTable() {
    const { marcaModelos, loading, error, eliminarMarcaModelo } = useMarcaModelos();
    const [selectedMarcaModelo, setSelectedMarcaModelo] = useState(null);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [searchTerm, setSearchTerm] = useState('');

    const handleEdit = (marcaModelo) => {
        setSelectedMarcaModelo(marcaModelo);
        setIsModalOpen(true);
    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    ...
                    console.error("Error al eliminar la marca modelo:", error);
                }
            }
        });
    };

    const filteredMarcaModelos = marcaModelos.filter((marcModel) => {
        const searchLower = searchTerm.toLowerCase();
        return(
            marcModel.nombre.toLowerCase().includes(searchLower)
        );
    });
            ...
            <div className="h-auto">
                ...
                <div className="flex overflow-x-auto mt-5 items-center justify-center">
                        ...
                        <div className="max-h-[1000px] w-full">
                            <table className="min-w-full bg-white rounded-lg border-t border-gray-300 table-auto">
                                ...
                                <tbody className="divide-y border-[1px] border-gray-300 divide-gray-200">
                                    {filteredMarcaModelos.length > 0 ? (
                                        filteredMarcaModelos.map((marca) => (
                                            <tr key={marca.id} className="text-xs sm:text-sm text-gray-700">
                    ...
                    <NewMarcaModelo
                        isOpen={isModalOpen}
                        onClose={closeModal}
                        marcaExistente={selectedMarcaModelo}
                        esEdicion={!!selectedMarcaModelo} />
                </div>
            </div>
        </div>
    )
...
export default MarcaModeloTable